...
class Director:
    ...
    def _run_planner(self):
        ...
        input_pictures = planner_data.get("input_pictures")
        input_setting = planner_data.get("input_setting")
        output = planner_data.get("output")
        input_note_path = os.path.join(self._target_name, WorkflowStep.RECORDER.value, input_note)
        input_pictures_path = os.path.join(self._target_name, input_pictures)
        ...
        planner = Planner(input_note_path=input_note_path, input_pictures_path=input_pictures_path, input_setting_path=input_setting_path, canon_path=canon_path, output_plan_path=output_path)
        planner.initialize()
        planner.run()
